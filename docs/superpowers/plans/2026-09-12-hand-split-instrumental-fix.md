...
- The "other" stem's measured pitch range (28-69) overlaps and can dip below the bass stem's range (28-56) at the bottom, so RH (transcribed from "other") can momentarily sit lower than LH (transcribed from bass, clamped to `HARD_LH_RANGE`) — a hand-crossing the old DP-based approach specifically penalized/avoided. The user's listening verdict preferred the stem-split sound overall despite this, but it's a real, named, unresolved risk the next "arrange-instrumental" pickup should inherit, not a claim that the new approach is strictly better in every respect.
