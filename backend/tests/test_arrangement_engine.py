...
def test_generate_lh_variants_produces_all_three_tiers():
    # duration=1.0s is a short chord (below Hard's SHORT_CHORD_THRESHOLD),
    # so Hard renders a block chord here rather than an arpeggio.
    ...
    assert len(list(variants.hard.flatten().notes)) == 3
