...
def quantize_part(part: stream.Part, grid: float, max_voices: Optional[int] = None) -> stream.Part:
    """Snap note onsets to the given grid (in quarterLength units),
    keeping at most max_voices notes per slot. With max_voices omitted
    (the default, and RH's only call site), keeps whichever note was
    encountered first per slot, exactly as before. Any explicit
    max_voices value — including 1 — keeps the highest-velocity notes
    among all notes whose onset falls in the slot instead (deduping
    repeated pitch classes to their higher-velocity instance first) —
    used for LH's polyphonic Easy/Medium reduction, where "first
    encountered" isn't a meaningful tie-break the way it is for a single
    melody line, even when reducing to one voice."""
    ...
    for slot in sorted(by_slot):
        ...
        kept = [candidates[0]] if max_voices is None else _cap_voices(candidates, max_voices)
        for element in kept:
